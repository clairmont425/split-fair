...
class _SavedConfigsSheetState extends State<SavedConfigsSheet> {
  ...
  void _save(AppState state) {
    ...
    final name = _nameCtrl.text.trim().isNotEmpty ? _nameCtrl.text.trim() : 'Config ${state.savedConfigs.length + 1}';
  ...
  Future<void> _simulatePurchase(AppState state) async {
    // TODO: replace with real in_app_purchase (product ID: split_fair_configs)
    await Future.delayed(const Duration(seconds: 1));
    if (mounted) {
      await state.unlockConfigsIap();
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Saved Configs unlocked!')));
    }
  }
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          if (configs.isNotEmpty) ...[
            ...
            Center(child: Column(children: [
              Icon(Icons.bookmark_border_rounded, size: 48, color: AppColors.textTertiary.withOpacity(0.4)),
              const SizedBox(height: 8),
              Text('No saved configs yet', style: Theme.of(context).textTheme.bodyMedium),
            ])),
            const SizedBox(height: 16),
          ],
        ]),
